...
module SBOM
  module CycloneDX
    Input = SchemaObject.build("Input") do
      ...
      prop :resource, ResourceReferenceChoice, required: -> { parameters.nil? && environment_vars.nil? && data.nil? }

      # Parameters - Inputs that have the form of parameters with names and values.
      prop :parameters, Set[Parameter], required: -> { resource.nil? && environment_vars.nil? && data.nil? }

      # Environment variables - Inputs that have the form of parameters with names and values.
      prop :environment_vars,
           Set[one_of: [Property, String]],
           required: -> { resource.nil? && parameters.nil? && data.nil? }

      # Data - Inputs that have the form of data.
      prop :data, Attachment, required: -> { resource.nil? && parameters.nil? && environment_vars.nil? }
