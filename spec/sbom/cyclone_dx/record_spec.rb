...
describe SBOM::CycloneDX::Record do
  ...
  describe "#as_json" do
    subject(:json_hash) { record.as_json }
    ...
    it "returns a JSON-compatible representation of the record" do
  ...
  describe "#to_json" do
    it "returns a JSON representation of the record" do
      expect(record.to_json).to eq(expected_json)
    end
  end
  ...
  context "when not used correctly" do
    it "raises an error when included in a non-Struct class" do
      expect do
        Class.new { include SBOM::CycloneDX::Record }
      end.to raise_error(ArgumentError, "Record must be included in a Struct")
    end

    it "raises an error when ClassMethods are extended by a non-Record class" do
      expect do
        Class.new(Struct.new(:field)) { extend SBOM::CycloneDX::Record::ClassMethods }
      end.to raise_error(ArgumentError, "This module can only be extended by a Record")
    end

    it "raises an error when calling valid? on a class that did not override it" do
      expect do
        Class.new(Struct.new(:field)) { include SBOM::CycloneDX::Record }.new.valid?
